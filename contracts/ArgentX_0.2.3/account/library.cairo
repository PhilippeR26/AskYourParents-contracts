...
from ArgentX.upgrade.Upgradable import _set_implementation
...
func execute_calls{syscall_ptr: felt*}(
    calls_len: felt, calls: Call*, reponse: felt*, index: felt
) -> (response_len: felt) {
    alloc_locals;
    ...
    let (response_len) = execute_calls(
        calls_len - 1, calls + Call.SIZE, reponse + res.retdata_size, index + 1
    );
