...
from starkware.starknet.common.syscalls import library_call, get_tx_info, get_contract_address
from ArgentX.plugins.IPlugin import IPlugin
from ArgentX.account.library import (
    Call,
    CallArray,
    Escape,
    ArgentModel,
...
func __validate__{
    ...
    if (call_array_len == 1) {
        ...
        if (call_array[0].to == tx_info.account_contract_address and
            call_array[0].selector == USE_PLUGIN_SELECTOR) {
...
func __execute__{
    ...
    let (retdata_len, retdata) = execute_call_array_plugin(
...
func execute_after_upgrade{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    // execute calls
    let (retdata_len, retdata) = execute_call_array(
        call_array_len, call_array, calldata_len, calldata
    );
